...
export default function Programmers() {
  ...
  const [programmers, changeProgrammers] = useState(listOfAwesome);
  const [programmerId, changeProgrammerId] = useState(null);
  ...
  if (programmerId) {style.color = "gold"}
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { changeProgrammerId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
